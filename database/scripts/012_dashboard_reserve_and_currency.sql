...
BEGIN
    ...
    DECLARE @gold decimal(19, 4) = 0;
    DECLARE @cash decimal(19, 4) = 0;
    DECLARE @deposits decimal(19, 4) = 0;

    SELECT TOP (1)
        @gold = COALESCE(GoldValue, 0),
        @cash = COALESCE(CashInHand, 0),
        @deposits = COALESCE(Deposits, 0)
    FROM dbo.ReserveSnapshots
    WHERE ReserveDate <= @AsOfDate AND DeletedTime IS NULL
    ORDER BY ReserveDate DESC, CreatedTime DESC;

    DECLARE @reserve decimal(19, 4) = @gold + @cash + @deposits;
    ...
    SELECT
        ...
        COALESCE(SUM(a.CurrentBalance), 0) AS Balance,
        COALESCE(SUM(CAST(a.CurrentBalance * r.RateToUsd AS decimal(19, 4))), 0) AS BalanceUsd
    FROM dbo.Currencies c
    JOIN dbo.CorrespondentAccounts a
        ON a.CurrencyId = c.CurrencyId AND a.IsDeleted = 0
    JOIN @rates r ON r.CurrencyId = c.CurrencyId
    WHERE c.IsDeleted = 0
      AND r.RateToUsd IS NOT NULL
    GROUP BY c.CurrencyId, c.CurrencyCode, c.CurrencyNameEn, c.CurrencyNameAr, c.Symbol
    HAVING COALESCE(SUM(CAST(a.CurrentBalance * r.RateToUsd AS decimal(19, 4))), 0) <> 0
