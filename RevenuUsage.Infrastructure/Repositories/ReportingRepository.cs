...
public sealed class ReportingRepository : IReportingRepository
{
    ...
    public async Task<IEnumerable<ObligationReportRow>> GetObligationsAsync(DateTime? f, DateTime? t, string? s, Guid? clientTypeId = null, CancellationToken ct = default)
    {
        await using var db = new SqlConnection(_cs);
